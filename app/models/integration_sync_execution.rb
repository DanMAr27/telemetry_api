...
class IntegrationSyncExecution < ApplicationRecord
  ...
  validate :validate_statistics_integrity, if: -> { completed? }
  ...
  def validate_statistics_integrity
    total = records_processed + records_failed + records_skipped

    if total != records_fetched
      errors.add(:base, "Las estadísticas no cuadran: procesados(#{records_processed}) + fallidos(#{records_failed}) + omitidos(#{records_skipped}) ≠ obtenidos(#{records_fetched})")
    end
  end
end
